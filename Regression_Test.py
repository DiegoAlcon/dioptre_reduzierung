...
model_bubble           = tf.keras.models.load_model('UNet_bubbles1.keras')
model_volume           = tf.keras.models.load_model('UNet_volume.keras')
model_segmen           = tf.keras.models.load_model('UNet_segmen.keras')

original_images_folder = r"C:\Users\SANCHDI2\OneDrive - Alcon\GitHub\dioptre_reduzierung\original" # Kleine rechner
original_files         = os.listdir(original_images_folder)
r                      = re.compile(r'\d+')
original_files.sort(key=lambda x: int(r.search(x).group()))
# Here should follow downsamplig but since images are predicted already in a really small dimension (128x128) is unecessary
...
predicted_masks_bubble = model_bubble.predict(x_test)
...
X_poly = poly.fit_transform(df_train[["bubbles", "volume", "segmen"]])
...
# Predictions
X_poly_test = poly.fit_transform(df_test[["bubbles", "volume", "segmen"]])
