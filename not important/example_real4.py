...
if __name__ == "__main__":
    ...
    #y = [round(elem) for elem in y]                                                                                                 #  TO ROUND ALL THE VALUES :)
    ...
    model.add(layers.Dense(1, activation='relu'))  # Single output neuron with tanh activation

    # Define callbacks
    #checkpoint = ModelCheckpoint(filepath='model_weights.keras', monitor='val_accuracy', save_best_only=True, verbose=1)
    #tensorboard = TensorBoard(log_dir='./logs', histogram_freq=1, write_graph=True, write_images=True)
    
    opt = keras.optimizers.Adam(learning_rate=0.1)
    model.compile(optimizer=opt, loss='mean_squared_error', metrics=['accuracy'])

    #history = model.fit(X_train, y_train, epochs=9, batch_size=32, validation_data=(X_val, y_val), callbacks=[checkpoint, tensorboard])
